...
public class FST extends SocialRecommender {
	...
	protected void initModel() throws Exception {
		...
		algoOptions = cf.getParamOptions("FST");
			...
			for (int u : trainMatrix.rows()) {
				...
				for (int i : ratedItems) {
					...
					for (int j : js) {
						...
						double pui = bi + regS*w_vi * sum_vi + (1-regS)*w_wi * sum_wi + wt * sum_ti;
						double puj = bj + regS*w_vj * sum_vj + (1-regS)*w_wj * sum_wj + wt * sum_tj;
						double ruj = 0;
	...
	public double predict(int u, int i) throws Exception {
		...
		return itemBias.get(i) + regS*wc * sum_c + (1-regS)*wr * sum_r + wt * sum_t;
	}
