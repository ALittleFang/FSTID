...
import main.java.librec.data.AddConfiguration;
...
import main.java.librec.data.SparseVector;
import main.java.librec.data.SymmMatrix;
...
public abstract class Recommender implements Runnable {
	...
	public enum Measure {
		...
		D5, D10, Pre5, Pre10, Rec5, Rec10,F1_M5,F1_M10, MAP, MRR, NDCG, AUC,
		/* execution time */
		TrainTime, TestTime,
		/* loss value */
		Loss
	...
	public static String getEvalInfo(Map<Measure, Double> measures) {
		...
		if (isRankingPred) {
			if (isDiverseUsed)
				...
				evalInfo = String.format("%.6f,%.6f,%.6f,%.6f", measures.get(Measure.Pre5),
						measures.get(Measure.Pre10), measures.get(Measure.F1_M5), measures.get(Measure.F1_M10));
...
//		}
		...
		Double pre5 = Stats.mean(precs5);
		Double pre10 = Stats.mean(precs10);
		measures.put(Measure.Pre5, pre5);
		measures.put(Measure.Pre10, pre10);

		Double rec5 = Stats.mean(recalls5);
		Double rec10 = Stats.mean(recalls10);

		Double f1_m5 = 2*pre5*rec5/(pre5+rec5);
		Double f1_m10 = 2*pre10*rec10/(pre10+rec10);
		measures.put(Measure.F1_M5, f1_m5);
		measures.put(Measure.F1_M10, f1_m10);
